...
export default function SearchMember() {
    ...
    useEffect(() => {
        if (searchResults) {
            ...
            if ((name && name.trim()) && (memberNumber && memberNumber.trim())) {
                ...
                .collection('users')
                .where(Filter.and(Filter('name', '==', name), Filter('memberNumber', '==', parseInt(memberNumber))))
                ...
                const subscriber = firestore()
                .collection('users')
                .where(Filter.or(Filter('name', '==', name), Filter('memberNumber', '==', parseInt(memberNumber))))
                .onSnapshot(querySnapshot => {
                    const members = [];
                    querySnapshot.forEach(documentSnapshot => {
                        members.push({ key: documentSnapshot.id, ...documentSnapshot.data() });
    ...
    const searchMember = async () => {
        ...
        if ((name && name.trim()) && (memberNumber && memberNumber.trim())) {
            ...
            .collection('users')
            .where(Filter.and(Filter('name', '==', name), Filter('memberNumber', '==', parseInt(memberNumber)))).get()
            .then(querySnapshot => {
                const members = [];
                querySnapshot.forEach(documentSnapshot => {
                    members.push({ key: documentSnapshot.id, ...documentSnapshot.data() });
            ...
            const snapshot = await firestore()
            .collection('users')
            .where(Filter.or(Filter('name', '==', name), Filter('memberNumber', '==', parseInt(memberNumber)))).get()
            .then(querySnapshot => {
                const members = [];
                querySnapshot.forEach(documentSnapshot => {
                    members.push({ key: documentSnapshot.id, ...documentSnapshot.data() });
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            ...
            onPress={() => {
                ...
                router.push({ pathname: '/(home)/(profile)/profile', params: { profileID: item.key } });
            }}>
